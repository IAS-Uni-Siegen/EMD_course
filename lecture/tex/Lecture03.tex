...
\begin{frame}
	...
	During the commutation time $\Delta t_\mathrm{c}$ the brush bridges two commutator segments and the short-circuited conductor coil current $i_\mathrm{c}$ is changing signs. Here, two major scenarios can be distinguished:
	\begin{itemize}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{column}{0.6\textwidth}
			...
			\end{equation}
			with the armature angular velocity $\omega$ and the armature diameter $d_\mathrm{a}$. Due to the changing current in the coil, the so-called reactane voltage $u_\mathrm{r}$ is induced:
			\begin{equation}
	...
	\frametitle{Induced voltage (cont.)}
	 ...
	 \begin{equation}
		 \begin{split}
			 ...
			 = \omega I_\mathrm{f}\frac{\mu_0 N_\mathrm{f} N_\mathrm{a} l_\mathrm{z} d_\mathrm{a} }{4 \delta p a \pi} \left[-\cos(\varepsilon)\right]_0^\pi\\[1em]
	...
	\frametitle{Power balance and efficiency (cont.)}
		...
		\begin{equation}
			\begin{split}
				...
				\eta_{\mathrm{gen}} &= \frac{P_\mathrm{el}}{P_\mathrm{me}} = \frac{u_\mathrm{a} i_\mathrm{a} + u_\mathrm{f} i_\mathrm{f}}{T \omega} = \frac{R_\mathrm{a}i_\mathrm{a}^2 + \omega L_\mathrm{f}' i_\mathrm{f} i_\mathrm{a} +R_\mathrm{f}i_\mathrm{f}^2}{L_\mathrm{f}' i_\mathrm{f}i_\mathrm{a}\omega}.
			\end{split}
		\end{equation}
		It can be noted that
		\begin{itemize}
			\item The machine parameters $R_\mathrm{a}$, $R_\mathrm{f}$, and $L_\mathrm{f}'$ are influencing the efficiency.
			\item The efficiency is a function of the load torque $T$ and the speed $\omega$, that is, depending on the operating point.
...
\begin{frame}
	...
	\begin{itemize}
		\item The neutral zone (field-free commutation area) is shifted by $\beta$ degrees in the circumferential direction, that is, exacerbate the commutation process (risk of sparking).
		\item High local field densities can lead to magnetic saturation which will increase the iron path reluctance and consequently decrease the machine's torque capability. Also, the iron losses will increase. 
		\item The imbalanced magnetic field leads to an imbalanced Lorentz force distribution on the armature conductors which can cause mechanical distortions. 
	\end{itemize}
\end{column}
\hfill
\begin{column}{0.45\textwidth}
...
\begin{frame}
   ...
   \begin{equation}
	...
	\end{itemize}
	In \eqref{eq:MMF_compensation_winding} $\alpha$ is only related to $\theta_\mathrm{a}$ as we assume the armature area to be bigger (or at least the same size) as the field pole (cf. \figref{fig:Compensation_winding_interpoles}). From \eqref{eq:MMF_compensation_winding} we can calculate the required compensation winding conductors
	\begin{equation}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.55\textwidth}
			...
			\end{equation}
			Applying the compensation winding design approach \eqref{eq:MMF_compensation_winding} results in:
			\begin{equation}
...
\begin{column}{0.45\textwidth}
	...
	\begin{figure}
		...
		\caption{Integration contour $\partial S$ and related MMF components for the interpole winding design (J.~B\"ocker, \textit{Elektrische Antriebstechnik}, Paderborn University, 2020)}
...
\begin{frame}
	...
	\begin{columns}
		...
		Assuming that the air gap reluctance is dominating the magnetic circuit, we  receive
			...
			\end{equation}
		The flux density in the interpole area is then
		\begin{equation}
			B_\mathrm{ip} = \mu_0 \frac{N_\mathrm{ip}  - N_\mathrm{a} (1-\alpha)}{2 \delta} i_\mathrm{a}.
		\end{equation}
		The comparison with \eqref{eq:Interpole_flux_density} reveals:
		\begin{equation}
			\begin{split}
				...
				\Leftrightarrow \quad &N_\mathrm{ip}  = N_\mathrm{a} (1-\alpha) + \frac{ L_\mathrm{c}\delta}{\mu_0 N_\mathrm{c} l_\mathrm{z} a w_\mathrm{b}}.
			\end{split}
		\end{equation}
\end{column}
\hfill
...
\begin{frame}
	\frametitle{Operation constraints: separately excited DC machine}
			...
			\end{equation}
			For sake of simplicity we only consider the first quadrant (cf. \figref{fig:machine_quadrants}), that is, positive torque and speed mode. From \eqref{eq:Operation_constraints_sep_DC_machine} $T\leq \psi_\mathrm{f}' I_\mathrm{max}$ follows. Also, the maximum speed is limited:
			\begin{equation}
...
\begin{frame}
	...
	\begin{columns}
		...
		\\[1em]
		Consider the sinusoidal excitation $$u(t) = \hat{u} \cos(\omega_\mathrm{el} t+ \varphi_{\mathrm{u}}) = \mathrm{Re}\left\{\hat{u} e^{\iu \omega_\mathrm{el} t + \varphi_{\mathrm{u}}}\right\},$$ which is represented by the complex phasor 
		\begin{equation}
