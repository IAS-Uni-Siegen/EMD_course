	...
	\begin{columns}
		\begin{column}{0.42\textwidth}
			\begin{itemize}
				...
				\item<2-> This is achieved by using a \hl{commutator} (brushes).
				\item<3-> Compared to homopolar machines, DC machines require a \hl{mechanical rectification} of the current.
			\end{itemize}
		\end{column}
		\hfill
...
\begin{frame}
	...
	\begin{itemize}
		\item<2-> If the commutation is fast: high local current densities are prevented.
		\item<3-> If the commutation is slow: high local current densities lead to sparking effects.
	\end{itemize}
	\begin{figure}
		\centering
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.42\textwidth}
			\begin{itemize}
				\item To reduce the effective length per armature conductor loop, the winding can form multiple \hl{pole pairs} $p$.
				...
				\item<4-> Given some inner stator diameter $d_\mathrm{s}$, the resulting \hl{pole pitch} is:
			\end{itemize}
...
\begin{frame}
	...
	For describing the \hl{armature winding layout}, the following parameters are introduced:
	\begin{gather*}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.65\textwidth}
			We introduce the following \hl{magnetic reluctances}
			\begin{equation}
				\begin{alignedat}{2}
...
\begin{frame}
	...
	\end{equation}\pause
	Here, $\delta$ is the air gap width and $A_\delta$ the \hl{effective cross-sectional area} of the air gap which is
	\begin{equation}
	...
	\begin{itemize}
		...
		\item $\alpha$ is the \hl{pole coverage}, that is, the ratio of the active pole surfaces to the pole pitch (cf. \figref{fig:Magnetic_field_normal_component_DC_machine} on next slide) representing the average field density in the air gap.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.45\textwidth}
			...
			\end{equation}\pause
			Assuming that the magnetic field only flows through each armature conductor in a perpendicular direction (cf. \figref{fig:DC_machine_cross_section_force_torque}), the absolute \hl{Lorentz force per armature conductor} is resulting in
			\begin{equation}
...
\begin{frame}
	...
	\begin{equation}
		...
		\end{split}
	\end{equation}
	With $\tau_\mathrm{p} = \pi d_\mathrm{s}/(2 p) = \pi d_\mathrm{a}/(2 p)$ assuming a very small air gap width $\delta$ (cf. \figref{fig:DC_machine_cross_section_two_pole_pairs}) we can also rewrite the \hl{machine torque} (a.k.a. air gap torque) as
	\begin{equation}
...
\begin{frame}
	...
	To write \eqref{eq:Torque_DC_machine} more compact, we introduce the \hl{effective field inductance}
	\begin{equation}
	...
	\end{equation} \pause
	Compared to the \hl{self-inductance} of the field winding
	\begin{equation}
		L_\mathrm{f} = \frac{N_\mathrm{f}^2}{2 R_\delta} = \frac{\mu  \alpha p \tau_\mathrm{p} l_z N_\mathrm{f}^2}{4 \delta},
	\end{equation} \pause
	we find an equivalent expression using the \hl{motor constant} $c$ as
	\begin{equation}
		L_\mathrm{f}' = \underbrace{\frac{2p}{a \pi}\frac{N_\mathrm{a}}{N_\mathrm{f}}}_{c} L_\mathrm{f}  = c L_\mathrm{f} .
	\end{equation} \pause
	Finally, we define the \hl{effective field flux linkage} $\psi_\mathrm{f}'$ to rewrite the torque expression
	\begin{equation}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.45\textwidth}
			...
			\end{equation}
			The \hl{flux linkage of a single armature coil} starting at position $\varepsilon$ is then
			\begin{equation*}
				\begin{split}
...
\begin{frame}
	...
	Assuming that the armature is rotating with the (constant) speed $n$ (or angular velocity $\omega = \dot{\varepsilon}$), the \hl{induced voltage per armature conductor loop} is
	\begin{equation}
	...
	\end{equation}
	To calculate the \hl{total induced voltage} $u_\mathrm{i}$, we consider
	\begin{itemize}
...
\begin{frame}
	...
	During the derivation of the DC machine model, we made \hl{several assumptions}:
	\begin{itemize}
...
\begin{frame}
	...
	Assuming a \hl{fixed excitation $\psi_\mathrm{f}'$} (e.g., by a permanent magnet or constant field current), the separately excited DC machine's armature voltage demand for a certain speed is:
	\begin{equation}
...
\begin{frame}
	...
	In the previous scenario, the no-load speed $\omega_0$ is limited by the maximum armature voltage $U_\mathrm{max}$. However, if the field winding current $I_\mathrm{f}$ is also controllable, the no-load speed can be increased by \hl{decreasing the excitation $\psi_\mathrm{f}'$} (so-called \hl{field weakening}). \pause Consider an armature operation both at the voltage and current constraint:
	\begin{equation}
...
\begin{frame}
	...
	\begin{align}
		i(t) & = \mathrm{Re}\left\{ \sqrt{2}\underline{I} e^{\iu\omega_\mathrm{el}t}\right\}\pause =\sqrt{2}\mathrm{Re}\left\{ \frac{U\left(R'(\omega)-\iu\omega_\mathrm{el}L\right)}{R'(\omega)^2+\omega_\mathrm{el}^2L^2} e^{\iu\omega_\mathrm{el}t}\right\} \pause \\
		...
		\begin{column}{0.55\textwidth}
			...
			\begin{equation*}
				\begin{split}
					...
					\onslide<2->{& = 2L_\mathrm{f}'\frac{U^2}{R'(\omega)^2+\omega_\mathrm{el}^2L^2}\cos\left(\omega_\mathrm{el}(t-\frac{L}{R'(\omega)})\right)^2               \\
