...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Winding characteristics}
	For describing the winding layout, the following parameters are introduced:
	\begin{gather*}
		Q: \mbox{number of slots}, \quad N_\mathrm{c}: \mbox{number of conductor turns per coil}, \\ K: \mbox{number of commutator elements}, \quad u = K/Q: \mbox{slot to commutator ratio}.
	\end{gather*}
    \begin{figure}
        \centering
...
\begin{frame}
	...
	\begin{itemize}
		\item The forward conductor of one coil and the return conductor of another coil are placed in the same slot.
    ...
    \begin{figure}
        ...
        \caption{Double layer winding with $u=3$ with a solid conductor element (which can be pre-manufactured for cost reasons)}
    \end{figure}
\end{frame}
    ...
    \begin{columns}
		\begin{column}{0.42\textwidth}
            \begin{itemize}
				...
				\item Front pitch $y_\mathrm{f}$: coil span from the front end
				\item Resultant pitch $y_\mathrm{r}$: distance between two consecutive coils
				\item Commutator pitch $y_\mathrm{c}$: distance between two consecutive commutator segments
			\end{itemize}
			\vspace{-0.4cm}
			\begin{varblock}{Progressive winding}
...
\begin{frame}
    ...
    \begin{columns}
		\begin{column}{0.42\textwidth}
			...
			\end{varblock}
			\begin{itemize}
				\item Retrogressive windings require more conductor material due to the crossing of the coils and, therefore, are less common.
				\item Technical feasibility requires $y_\mathrm{b} - y_\mathrm{f} = \pm y_\mathrm{c}$, i.e., the lap winding progresses or retrogresses by one commutator element.
			\end{itemize}
		\end{column}
        \hfill
		\begin{column}{0.55\textwidth}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.35\textwidth}
			\begin{itemize}
				\item Total number of armature turns: $N = \frac{KN_\mathrm{c}}{2p}$
				\item Current per armature conductor: $i_\mathrm{c} = \frac{I_\mathrm{A}}{2 p}$
			\end{itemize}
			\begin{varblock}{Parallel connection of poles}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.35\textwidth}
			\begin{itemize}
				\item Total number of armature turns: $N = \frac{K N_\mathrm{c}}{2}$
				\item Current per armature conductor: $i_\mathrm{c} = \frac{I_\mathrm{A}}{2}$
			\end{itemize}
			\begin{varblock}{Series connection of poles}
...
\begin{frame}
	...
	\end{equation}
	we can wrap up the following summary:
	\begin{equation}
		\mbox{Current per conductor: } I_\mathrm{c} = \frac{I_\mathrm{a}}{2 a}, \quad \mbox{Total number of armature turns: } N_\mathrm{a} = \frac{K N_\mathrm{c}}{2 a}.
	\end{equation}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.65\textwidth}
			\textbf{Assumption}: The air gap field distribution is homogenous and without any leakage (cf. \figref{fig:Ideal_air_gap_field}).\\ Consequently, we model the magnetic machine behavior with the simplified network shown in \figref{fig:Simplified_magnetic_network_DC_machine}. 
			\begin{figure}
				\centering
		...
		\begin{column}{0.65\textwidth}
			...
			\begin{equation}
				\begin{alignedat}{2}
					...
					R_\delta &= \frac{l_\delta}{\mu_0 A_\delta} \quad  &&(\mbox{air gap reluctance}).
				\end{alignedat}
			\end{equation}
			The above $l_i$ and $A_i$ are the respective lengths and cross-sectional areas of the field paths. Furthermore, we have 
		...
		\begin{column}{0.65\textwidth}
			...
			\begin{equation}
				...
				\phi_\delta &= \frac{N_\mathrm{F} I_\mathrm{F}}{2 R_\delta + R_\mathrm{A} + \frac{1}{2}R_\mathrm{S}}\\
							& = \frac{N_\mathrm{F} I_\mathrm{F}}{\mu_0}\left(2\frac{l_\delta}{A_\delta} + \frac{l_\mathrm{A}}{\mu_{\mathrm{r, Fe}} A_\mathrm{A}} + \frac{1}{2}\frac{l_\mathrm{S}}{\mu_{\mathrm{r, Fe}} A_\mathrm{S}}\right)^{-1}. 
			\end{split}
			\label{eq:Air_gap_flux_DC_machine_simple}
			\end{equation}
			While the relative permeability of the iron paths is depending on the magnetic flux ($\mu_{\mathrm{r, Fe}} = \mu_{\mathrm{r, Fe}}(\phi)$) due to saturation (cf. \figref{fig:Permeability_of_ferromagnet}) rendering \eqref{eq:Air_gap_flux_DC_machine_simple} a nonlinear equation, we will assume that the air gap reluctance is dominating 
			\begin{equation}
...
\begin{frame}
	...
	\begin{equation}
		\phi_\delta = \frac{N_\mathrm{F} I_\mathrm{F}}{2 R_\delta} = \frac{N_\mathrm{F} I_\mathrm{F}}{2 \frac{l_\delta}{\mu_0 A_\delta}} = \frac{\mu_0 N_\mathrm{F} I_\mathrm{F} A_\delta}{2 l_\delta}.
		\label{eq:Air_gap_flux_DC_machine}
	\end{equation}
	Here, $l_\delta$ is the air gap width and $A_\delta$ the effective cross-sectional area of the air gap which is
	\begin{equation}
	...
	\begin{itemize}
		\item $l_z$ is the axial length of the machine.
		\item The air gap width is very small such that the pole pitch $\tau_\mathrm{p}$ can be used as a good approximation for the air gap width in circumferential direction.
...
\begin{frame}
	...
	\begin{equation}
		B_\delta = \frac{\phi_\delta}{A_\delta} = \frac{\mu_0 N_\mathrm{f} I_\mathrm{f}}{2 l_\delta p}.
		\label{eq:Air_gap_flux_density_DC_machine}
	\end{equation}
	The absolute Lorentz force per armature conductor is resulting in
	\begin{equation}
		F_\mathrm{c} =  B_\delta l_z I_\mathrm{c}= \frac{\mu_0 N_\mathrm{f} l_z I_\mathrm{f} I_\mathrm{a}  }{4 l_\delta p a}.
	...
	\begin{equation}
		T_\mathrm{c} = F_\mathrm{c} \frac{d_\mathrm{a}}{2} = \frac{\mu_0 N_\mathrm{f} l_z d_\mathrm{a} I_\mathrm{f} I_\mathrm{a} }{8 l_\delta p a}.
	...
	\begin{equation}
		T = 2 \alpha N_\mathrm{a} T_\mathrm{c} = \frac{\mu_0 \alpha N_\mathrm{f} N_\mathrm{a} l_z d_\mathrm{a}}{4 l_\delta p a} I_\mathrm{f} I_\mathrm{a} .
