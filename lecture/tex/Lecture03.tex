...
\begin{frame}
	...
	The torque per conductor for an armature diameter $d_\mathrm{a}$ is
	\begin{equation}
	...
	\begin{equation}
		\begin{split}
			\onslide<2->{T 	&= 2 \alpha N_\mathrm{a}  T_\mathrm{c} }\\ 
				\onslide<3->{&= \frac{\mu_0 \alpha N_\mathrm{f} N_\mathrm{a} l_z d_\mathrm{a}}{4 \delta p a} I_\mathrm{f} I_\mathrm{a}} 
		\end{split}
	\end{equation}
\end{frame}
...
\begin{frame}
	...
	\begin{equation}
		\begin{split}
		T &= \frac{\mu_0 \alpha N_\mathrm{f} N_\mathrm{a} l_z d_\mathrm{a}}{4 \delta p a} I_\mathrm{f} I_\mathrm{a}\\
		&= \frac{\mu_0 \alpha N_\mathrm{f} N_\mathrm{a} l_z \tau_\mathrm{p}}{2 \pi  \delta a} I_\mathrm{f} I_\mathrm{a}.
		\end{split}
...
\begin{frame}
	...
	\end{equation} 
	Above, $A_\mathrm{c}$ is the cross-sectional area of an armature conductor loop (cf. \figref{eq:Torque_DC_machine}). \pause Assuming that the armature is rotating with the speed $n$ (or angular velocity $\omega = 2 \pi n=\dot{\varepsilon}$), the induced voltage per armature conductor loop is
	\begin{equation}
		\begin{split}
