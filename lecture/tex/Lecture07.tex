...
\begin{frame}
	...
	\begin{equation}
		\begin{split}
			...
			= R_\mathrm{s} \underline{I}_\mathrm{s} + \mathrm{j} \underbrace{\omega_\mathrm{s} \left(L_{\sigma,\mathrm{s}} + M\right)}_{X_\mathrm{s}}\underline{I}_\mathrm{s} + \underbrace{\mathrm{j}\omega_\mathrm{s} M I_\mathrm{f}}_{\underline{U}_\mathrm{i}}
		\end{split}
		\label{eq:SM_voltage_steady_state}
	\end{equation}
	with $\underline{U}_\mathrm{i}$ being the internal voltage, i.e., the induced voltage due to the field winding excitation and $X_\mathrm{s}$ being the synchronous reactance (which can be empirically identified using open-circuit and short-circuit tests).
\end{frame}
...
\begin{frame}
	\frametitle{Steady-state behavior (cont.)} 
	The ECD of the cylindrical SM is sown in \figref{fig:SM_ECD_steady_state}. Here, the following can be noted:
	\begin{itemize}
...
\begin{frame}
	...
	\end{equation}
	Here, the stator current is zero. On the other hand, the short-circuit current is given by
	\begin{equation}
		\underline{I}_\mathrm{s,sc} = \frac{\underline{U}_\mathrm{i}}{\mathrm{j}X_\mathrm{s}} = \frac{\omega_\mathrm{s} M I_\mathrm{f}}{\mathrm{j}\omega_\mathrm{s}\left(L_{\sigma,\mathrm{s}} + M\right)} = \frac{M}{\mathrm{j}\left(L_{\sigma,\mathrm{s}} + M\right)}I_\mathrm{f}.
	\end{equation}
	Here, the stator voltage is zero and one can observe that the short-circuit current $I_\mathrm{f}$ can be interpreted as the excitation current converted via the inductance ratio. Finally, the synchronous reactance can be calculated by the ratio of the open-circuit voltage and the short-circuit current:
	\begin{equation}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.55\textwidth}
			\begin{itemize}
				\item From \eqref{eq:SM_torque_steady_state} we see that the torque depends on $\sin(\theta)$.
				\item Beyond $\theta = \pm 90^\circ$, the absolute torque decreases again.
				\item If the SM is operated with a fixed stator excitation, e.g., by a stiff grid voltage, the load angle $\theta$ is determined by the mechanical load.
				\item If the absolute mechanical load is increased such that $|\theta| > 90^\circ$ applies, the SM will lose synchronism and stall.
				\item Hence, the stable operation range is limited to $|\theta| \leq 90^\circ$ (while in practice an additional safety margin is considered).
			\end{itemize}
        \end{column}
        \begin{column}{0.45\textwidth}
            \begin{figure}
