...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.48\textwidth}
			\begin{itemize}
				\item Magnetic domains are regions within a material where the magnetic moments of atoms are aligned (\enquote{mini magnets}).
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item<3-> This process requires work and leads to heat dissipation (\hl{hysteresis losses}).
		...
		\begin{column}{0.475\textwidth}
			...
			\begin{figure}
				...
				\caption{Exemplary hysteresis curve with $B_\mathrm{r}$ being the \hl{remanence field density} and $H_\mathrm{c}$ the \hl{coercivity field strength}}
	...
	\begin{columns}
		\begin{column}{0.4\textwidth}
			\begin{itemize}
				...
				\item<2-> The coercivity $H_\mathrm{c}$ and the remanence $B_\mathrm{r}$ decrease with increasing temperature.
				\item<3-> Hence, with higher temperatures, a PM gets more susceptible to demagnetization.
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{figure}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.6\textwidth}
			...
			\begin{align}
				\mbox{Integral form:} \quad     & \oint_{\partial S} \bm{E} \cdot \mathrm{d}\bm{s} = -\frac{\mathrm{d}}{\mathrm{d}t}\iint_{S}\bm{B}\cdot\mathrm{d}\bm{S},\label{eq:induction_law_int} \\
			...
			\begin{itemize}
				\item<2-> \hl{Lentz's law}: The induced electric field opposes the change in magnetic field (negative sign above).
			\end{itemize}
		\end{column}
		\hfill
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.55\textwidth}
			If the integration path $\partial S$ is identical to a \hl{conductor loop}, the changing magnetic field induces a voltage $u_\mathrm{i}$ (\hl{electromotive force}, EMF) according to Faraday's law:
			\begin{align}
			...
			\begin{itemize}
				...
				\item<3-> The magnetic flux per element is:
				      \begin{equation}
					      \phi_k = A_k B_k .
				      \end{equation}
				\item<4-> The magnetic voltage (\hl{magnetomotive force} -- MMF) per element is:
				      \begin{equation}
					      \theta_k = l_k H_k .
				      \end{equation}
			\end{itemize}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.575\textwidth}
			\begin{itemize}
				\item The \hl{magnetic reluctance} per element is:
				      \begin{equation}
					      R_k = \frac{\theta_k}{\phi_k} = \frac{l_k}{\mu_0\mu_{\mathrm{r}k}A_k}.
				      \end{equation}
				\item<2-> The \hl{magnetic conductivity} (or \hl{permeance}) per element is:
				      \begin{equation}
...
\begin{frame}
	...
	\begin{table}
		...
		\begin{tabular}{lcclcc}
			...
			\onslide<2->{Electric field        & $\bm{E}$                                 & \si{\volt\per\metre}    & Magnetic field            & $\bm{H}$                                      & \si{\ampere\per\metre}} \\
			\onslide<3->{Current               & $i$                                      & \si{\ampere}            & Magnetic flux             & $\phi$                                        & \si{\volt\second}}      \\
			\onslide<4->{Resistance            & $R$                                      & \si{\ohm}               & Reluctance                & $R$                                           & \si{\per\henry}}        \\
			\onslide<5->{Conductance           & $G$                                      & \si{\siemens}           & Permeance                 & $\Lambda$                                     & \si{\henry}}            \\
			\onslide<6->{Conductivity          & $\sigma$                                 & \si{\siemens\per\metre} & Permeability              & $\mu$                                         & \si{\henry\per\metre}}  \\
			\onslide<7->{Ohm's law             & $u = R i$                                &                         & Hopkinson's law           & $\theta = R \phi$                             & }                       \\
			\onslide<8->{Kirchoff's first law  & $\sum i_k = 0$                           &                         & Equivalent first law      & $\sum \phi_k = 0$                             & }                       \\
			\onslide<9->{Kirchoff's second law & $\sum u_k = 0$                           &                         & Equivalent second law     & $\sum \theta_k - \theta_0 = 0$                & }                       \\
			\bottomrule
		\end{tabular}
...
\begin{frame}
	...
	\begin{figure}
		...
		\begin{tikzpicture}[
			...
			child {node[level 2] (c1) {Winding losses}}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item In bulky conductive materials (e.g., electromagnetic steel) this voltage drives currents called \hl{eddy currents}.
				\item<2-> Eddy currents lead to losses and heat dissipation.
				\item<3-> To reduce eddy currents, \hl{laminated cores} are used as they decrease the effective current path width and, therefore, increase the effective resistance per sheet.
			\end{itemize}
		\end{column}
		\hfill
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\end{varblock}
			\onslide<2->{From \eqref{eq:induction_law_int} integrating over $S$, we get
				$$
...
\begin{frame}
	...
	$$\pause
	The average power loss per volume (considering the entire sheet thickness) is:
	$$
...
\begin{frame}
	\frametitle{Eddy currents: single sheet example (cont.)}
	The \hl{average power loss density per volume and time} is then:
	$$
