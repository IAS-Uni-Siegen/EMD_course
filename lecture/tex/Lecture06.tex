...
\begin{frame}
	...
	\begin{figure}
		...
		\begin{subfigure}{0.49\textwidth}
			\centering
			\includegraphics[width=0.8\textwidth]{fig/lec06/Squirrel_cage.jpg}
...
\begin{frame}
    ...
    $$
    Assuming that the stator ohmic voltage drop is negligible ($R_\mathrm{s}=0$), the stator voltage equation from \eqref{eq:IM_model_voltage_equations_steady_state_slip_ratio} reveals
    \begin{equation}
    ...
    \end{equation}
    Hence, the stator voltage phasor is purely imaginary and the stator flux phasor is real due to the chosen orientation.  Based on the torque expression \eqref{eq:IM_model_k_torque_stator_flux_orientation} the next steps focus on finding the stator current phasor $\underline{I}_\mathrm{s}$. 
\end{frame}
...
\begin{frame}
    ...
    Inserting \eqref{eq:IM_model_stator_flux_orientation_steady_state} in
     $$
     \underline{\Psi}_\mathrm{s} = (L_{\sigma,\mathrm{s}} +M) \underline{I}_\mathrm{s} +  M\underline{I}'_\mathrm{r}
     $$
     delivers
     \begin{equation}
        \underline{I}'_\mathrm{r} = \frac{1}{M}\left[\frac{U_\mathrm{s}}{\omega_\mathrm{s}}-(L_{\sigma,\mathrm{s}} +M)\underline{I}_\mathrm{s}\right].
        \label{eq:IM_model_rotor_current_steady_state}
     \end{equation}
     Futhermore, the rotor voltage for the squirrrel cage IM is
     $$
...
\begin{frame}
	\frametitle{Squirrel cage IM torque-speed characteristic (cont.)} 
    Inserting \eqref{eq:IM_model_rotor_flux_steady_state} into $$ \underline{\Psi}'_\mathrm{r} = (L'_{\sigma,\mathrm{r}} + M) \underline{I}'_\mathrm{r} +  M \underline{I}_\mathrm{s}$$
    yields
    \begin{equation}
        \frac{\mathrm{j}}{\omega_\mathrm{s}}\frac{R'_\mathrm{r}}{s}\underline{I}'_\mathrm{r} = (L'_{\sigma,\mathrm{r}} + M) \underline{I}'_\mathrm{r} +  M \underline{I}_\mathrm{s} \quad \Leftrightarrow \quad \underline{I}'_\mathrm{r} = -\frac{M\left[(L'_{\sigma,\mathrm{r}} + M)(s \omega_\mathrm{s})^2  + \mathrm{j}R'_\mathrm{r}(s \omega_\mathrm{s})\right]}{(L'_{\sigma,\mathrm{r}} + M)^2(s \omega_\mathrm{s})^2 + (R'_\mathrm{r})^2}\underline{I}_\mathrm{s}
    \end{equation}
    Combining this with \eqref{eq:IM_model_rotor_current_steady_state} results in
    \begin{equation}
         -\frac{M\left[(L'_{\sigma,\mathrm{r}} + M)(s \omega_\mathrm{s})^2  + \mathrm{j}R'_\mathrm{r}(s \omega_\mathrm{s})\right]}{(L'_{\sigma,\mathrm{r}} + M)^2(s \omega_\mathrm{s})^2 + (R'_\mathrm{r})^2}\underline{I}_\mathrm{s} = \frac{1}{M}\left[\frac{U_\mathrm{s}}{\omega_\mathrm{s}}-(L_{\sigma,\mathrm{s}} +M)\underline{I}_\mathrm{s}\right]
    \end{equation}
    Solving for $\underline{I}_\mathrm{s}$ yields...
\end{frame}
