...
B_a = (
    B_hat / 2 * (np.sin(theta_vec * p - omega * t) + np.sin(theta_vec * p + omega * t))
)
B_b = (
    B_hat
    / 2
    * (
        np.sin(theta_vec * p - omega * t)
        + np.sin(theta_vec * p + omega * t - 4 * np.pi / 3)
    )
)
B_c = (
    B_hat
    ...
    * (
        np.sin(theta_vec * p - omega * t)
        + np.sin(theta_vec * p + omega * t - 8 * np.pi / 3)
    )
)
...
for i in range(3):
    ax[i].set_ylabel(
        r"$B^{(1)}_\mathrm{" + chr(ord("a") + i) + r"}(\vartheta)/\hat{B}_\delta$"
    )
