...
parser = argparse.ArgumentParser(
    description="Create animation of rotating field",
    epilog=epilog,
...
parser.add_argument(
    "-c", "--compose", action="store_true", help='vectors are plotted "chained"'
)
parser.add_argument(
    "-r",
    "--resulting",
...
parser.add_argument(
    "-f", "--frames", type=int, default=50, help="total number of frames"
)
parser.add_argument(
    "-o", "--output", default="output_file", help="output file (without extension)"
)
...
CENTERFIG = (
    614,
    763,
)  # Coordinates of the center of the figure (for rotating field vectors)
VL = 135  # Vector length
VHL = 45  # Vector arrow size
DRAWV = (
    [args.coil1, args.coil2, args.coil3] if not args.all else [True, True, True]
)  # Which coils to plot
...
def draw_triangle(ax, angle, color="red", size=20):
    ...
    p_tr = [
        (
            p[0] * math.cos(angle) - p[1] * math.sin(angle) + CENTERFIG[0],
            p[0] * math.sin(angle) + p[1] * math.cos(angle) + CENTERFIG[1],
        )
        for p in points
    ]
...
for i in range(NUMBER_OF_TIME_STEPS):
    ...
    for j, ax in enumerate(axp):
        ...
        ax.arrow(
            0, 0, 2 * np.pi * 0.98, 0, head_width=0.1, head_length=0.2, fc="k", ec="k"
        )
        ax.arrow(
            0,
            -1,
        ...
        fig.sca(ax)
        y = np.cos(
            np.linspace(0, 2 * np.pi, 500) - j * 2 * np.pi / 3
        )  # Sinusoidal wave for each coil
        ...
        if DRAWV[j]:
            ...
            if DRAW_ARROWS:
                plt.arrow(
                    x1, y1, dx, dy, color=styles[j], head_width=VHL, head_length=VHL
                )
            if COMPOSE:
                x1 += (VL * yp) * np.cos(j * 2 * np.pi / 3) + np.sign(
                    yp
                ) * VHL * np.cos(j * 2 * np.pi / 3)
                y1 += (VL * yp) * np.sin(-j * 2 * np.pi / 3) + np.sign(
                    yp
                ) * VHL * np.sin(-j * 2 * np.pi / 3)
    ...
    if DRAWRES:
        ...
        plt.arrow(
            CENTERFIG[0],
            CENTERFIG[1],
            x1 - CENTERFIG[0] - np.sign(yp) * VHL * np.cos(j * 2 * np.pi / 3),
            y1 - CENTERFIG[1] - np.sign(yp) * VHL * np.sin(-j * 2 * np.pi / 3),
            color="k",
            head_width=VHL,
            head_length=VHL,
            linewidth=2,
